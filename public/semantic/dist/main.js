		...
		$.ajax({
			...
			success: function(response) {
				...
				if(this.url.replace('/', '') == 'homepage') {
					...
					history.pushState({}, '', this.url.replace('/', ''));
					$('html, body').animate({
						scrollTop: ($(window).width() > 766) ? ((source == "homepage") ? 321 : 0) : 0
					}, 'slow', function() {
	...
	setTimeout(function() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$('.menu.right .item').removeClass('active');
				$('.menu.right .item[href="' + this.url.replace('/', '') +'"]').addClass('active');
				$('.mobilemenu li').removeClass('active');
				$('.mobilemenu .' + this.url.replace('/', '') + '-item').addClass('active');
			}
		});
	}, 3000);
